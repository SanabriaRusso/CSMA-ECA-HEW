...
my @inputData;
my $highStickiness = 0;
my $lowStickiness = 10000;
my $outputFile = 'Results/processed_varStickiness.txt';
...
while (my $row = <$fh>)
{
    if($row !~ /^#/)
    {
        ...
        $highStickiness = $inputData[35]
            if ($highStickiness < $inputData[35]);
        $lowStickiness = $inputData[35]
            if ($lowStickiness > $inputData[35]);
    }
...
#66 droppedVO           #68 channelError                #70 Stickiness\n");


OUTTER: foreach($lowStickiness .. $highStickiness)
{
    open(my $fh, "<", $filename)
        or die "Could not open file $filename $!";
